		...
		if (data == ''){
			...
			for (var row in data) {
				...
				table1.row.add([data[row].local.name, codeview,data[row].ownerName,d1,data[row].censusTaker.name,data[row].inhabited,
					...
					data[row].noSproomsReasons,data[row].sleep,data[row].mosqSinInsecticida,data[row].latitude,data[row].longitude,data[row].altitud,data[row].exactitud,data[row].obs,data[row].pasive,data[row].recordUser,d2,btnview]);
			}
			$('#viviendasdiv').show();
		}
		$.unblockUI();
