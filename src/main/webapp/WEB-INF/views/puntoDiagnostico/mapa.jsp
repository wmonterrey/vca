...
<body class="app header-fixed sidebar-fixed aside-menu-fixed aside-menu-hidden">
      ...
      <div class="animated fadeIn">
          <div class="card">
            ...
            <div class="card-body">
              ...
              <spring:url value="/admin/puntos/"	var="viewList"/>	
              ...
              <div class="row float-right mr-4" >
              	<button id="pdxs_list" onclick="location.href='${fn:escapeXml(viewList)}'" type="button" class="btn btn-outline-primary"><i class="fa fa-list-ul"></i>&nbsp; <spring:message code="list" /></button>
              </div>
              <br><br>
  ...
  <script>
		...
		<c:forEach var="puntoDiagnostico" items="${puntoDiagnosticos}">
			var miLat = "${puntoDiagnostico.latitude}";
			var miLong = "${puntoDiagnostico.longitude}";
			
			if(!(miLat == "" || miLong == "")){
				if("${puntoDiagnostico.pasive}"=='1'.charAt(0)){
					theMarker = L.marker(["${puntoDiagnostico.latitude}", "${puntoDiagnostico.longitude}"],{url: "${viewList}"+"/"+"${puntoDiagnostico.ident}"+"/"}).addTo(mymap).setIcon(redIcon).on('click', onClick);
				}
				else{
					theMarker = L.marker(["${puntoDiagnostico.latitude}", "${puntoDiagnostico.longitude}"],{url: "${viewList}"+"/"+"${puntoDiagnostico.ident}"+"/"}).addTo(mymap).setIcon(blueIcon).on('click', onClick);
				}
				theMarker.addTo(locMarkers);
				theMarker.bindTooltip("${puntoDiagnostico.clave}");
			}
		</c:forEach>
		...
		legend.onAdd = function(map) {
		  ...
		  div.innerHTML += "<h4><spring:message code="pdxs" /></h4>";
