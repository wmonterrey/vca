...
<body class="app header-fixed sidebar-fixed aside-menu-fixed aside-menu-hidden">
      ...
      <div class="animated fadeIn">
          <div class="card">
            ...
            <div class="card-body">
              ...
              <spring:url value="/admin/criaderos/"	var="viewList"/>	
              ...
              <div class="row float-right mr-4" >
              	<button id="criaderos_list" onclick="location.href='${fn:escapeXml(viewList)}'" type="button" class="btn btn-outline-primary"><i class="fa fa-list-ul"></i>&nbsp; <spring:message code="list" /></button>
              </div>
              <br><br>
  ...
  <script>
		...
		<c:forEach var="criadero" items="${criaderos}">
			var miLat = "${criadero.latitude}";
			var miLong = "${criadero.longitude}";
			
			if(!(miLat == "" || miLong == "")){
				if("${criadero.pasive}"=='1'.charAt(0)){
					theMarker = L.marker(["${criadero.latitude}", "${criadero.longitude}"],{url: "${viewList}"+"/"+"${criadero.ident}"+"/"}).addTo(mymap).setIcon(redIcon).on('click', onClick);
				}
				else{
					theMarker = L.marker(["${criadero.latitude}", "${criadero.longitude}"],{url: "${viewList}"+"/"+"${criadero.ident}"+"/"}).addTo(mymap).setIcon(blueIcon).on('click', onClick);
				}
				theMarker.addTo(locMarkers);
				theMarker.bindTooltip("${criadero.info}");
			}
		</c:forEach>
		...
		legend.onAdd = function(map) {
		  ...
		  div.innerHTML += "<h4><spring:message code="criaderos" /></h4>";
